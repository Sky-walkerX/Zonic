...
app.get('/login', (req, res) => {
  ...
  const scope = ['user-read-private', 'user-read-email', 'user-library-read', 'playlist-read-private', 'playlist-read-collaborative', 'user-top-read'].join(' ');
...
app.post('/refresh_token', async (req, res) => {  //refresh the user's access token when it expires using a refresh token without needing the user to log in again.
    const { refresh_token } = req.body;
    if (!refresh_token && !storedRefreshToken) { // Check body first, then internal store
...
app.get('/user', requireAuth, async (req, res) => {
    try {
        const response = await axios.get('https://api.spotify.com/v1/me', {  //user info
            headers: { Authorization: `Bearer ${req.token}` }
        });
        res.json(response.data);
    } catch (error) {
        console.error('/user Error:', error.response?.status, error.response?.data?.error?.message);
        res.status(error.response?.status || 500).json({ error: error.response?.data?.error?.message || "Failed fetching user data" });
    }
});
...
app.get('/playlists', requireAuth, async (req, res) => {
    try {
        ...
        const response = await axios.get('https://api.spotify.com/v1/me/playlists', { //user's playlist info
            headers: { Authorization: `Bearer ${req.token}` },
            params: { limit }
        });
        ...
        res.status(error.response?.status || 500).json({ error: error.response?.data?.error?.message || "Failed fetching playlists" });
    }
});
...
app.get('/liked-songs', requireAuth, async (req, res) => {
    try {
        ...
        const response = await axios.get('https://api.spotify.com/v1/me/tracks', { //liked songs info
            headers: { Authorization: `Bearer ${req.token}` },
            params: { limit, offset }
        });
        res.json(response.data.items || []);
        ...
        res.status(error.response?.status || 500).json({ error: error.response?.data?.error?.message || "Failed fetching liked songs" });
    }
});
...
app.get('/top-tracks', requireAuth, async (req, res) => {
    try {
        ...
        const response = await axios.get('https://api.spotify.com/v1/me/top/tracks', { //most listened (top-tracks) info
            headers: { Authorization: `Bearer ${req.token}` },
        ...
        res.status(error.response?.status || 500).json({ error: error.response?.data?.error?.message || "Failed to fetch top tracks" });
    }
});
...
app.get('/api/search', async (req, res) => {
    ...
    try {
        ...
        console.error("Google Search API Error:", error.response?.status, error.response?.data?.error?.message || error.message);
